...
contract SimplePool {
    ...
    function addLiquidity(uint256 tokenAmount) external payable {
        ...
        uint256 addLiquidityBefore = token.balanceOf(address(this));
        token.transferFrom(msg.sender, address(this), tokenAmount);
        require(token.balanceOf(address(this)) == addLiquidityBefore + tokenAmount ,
        "addLiquidity transferFrom is fail");  //修复假充值， OpenZeppelin 的 SafeERC20 也是这个思路实际可以照搬
